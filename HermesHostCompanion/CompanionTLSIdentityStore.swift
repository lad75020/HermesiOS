...
final class CompanionTLSIdentityStore {
    ...
    private func bootstrapIfNeeded() throws {
        ...
        if fileManager.fileExists(atPath: caCertificateURL().path) == false || fileManager.fileExists(atPath: caPrivateKeyURL().path) == false {
            try generateLocalCA()
        }

        if fileManager.fileExists(atPath: serverP12URL().path) == false || fileManager.fileExists(atPath: serverCertificateURL().path) == false {
