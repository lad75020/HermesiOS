...
final class CompanionPhotonGatewayTests: XCTestCase {
    ...
    private func fixture(_ body: (URL, URL, CompanionGatewayRegistry) async throws -> Void) async throws {
        ...
        try await body(root, selected, registry)
    }

    func testCatalogListsPhotonWithAllowlistedWriteOnlyFields() async throws {
        try await fixture { root, _, registry in
            let loaded = try await registry.config(workspacePath: root.path, profileName: "selected")
            ...
            for key in keys {
                ...
                let result = try await registry.setEnv(workspacePath: root.path, profileName: "selected", key: key, value: "fixture-replacement")
            ...
            do {
                _ = try await registry.setEnv(workspacePath: root.path, profileName: "selected", key: "PHOTON_API_KEY", value: "fixture")
                XCTFail("A non-allowlisted key must be rejected.")
            } catch {}
            do {
                _ = try await registry.setEnv(workspacePath: root.path, profileName: "selected", key: "PHOTON_PROJECT_SECRET", value: "[configured]")
                XCTFail("The write-only placeholder must not replace a secret.")
            } catch {}
        }
    }

    func testToggleAndSecretPersistenceAreRestrictedToSelectedProfile() async throws {
        try await fixture { root, selected, registry in
            ...
            for enabled in [false, true, false] {
                let result = try await registry.setPlatformEnabled(workspacePath: root.path, profileName: "selected", platform: "photon", enabled: enabled)
                XCTAssertEqual(result.profilePath, selected.path)
                XCTAssertEqual(result.platformEnabled["photon"], enabled)
                let reloaded = try await registry.config(workspacePath: root.path, profileName: "selected")
            ...
            _ = try await registry.setEnv(workspacePath: root.path, profileName: "selected", key: "PHOTON_PROJECT_SECRET", value: replacement)
    ...
    func testTraversalAndSymlinkProfilesCannotWriteOutsideScope() async throws {
        try await fixture { root, selected, registry in
            ...
            for name in ["../selected", "selected/../../", "escape", "missing"] {
                do {
                    _ = try await registry.setEnv(workspacePath: root.path, profileName: name, key: "PHOTON_PROJECT_SECRET", value: "fixture-new")
                    XCTFail("Invalid profile \(name) must be rejected.")
                } catch {}
                do {
                    _ = try await registry.setPlatformEnabled(workspacePath: root.path, profileName: name, platform: "photon", enabled: false)
                    XCTFail("Invalid profile \(name) must be rejected.")
                } catch {}
            }
    ...
    func testMalformedYAMLIsNeverReplacedByToggle() async throws {
        try await fixture { root, selected, registry in
            ...
            for text in ["gateway: [broken", "gateway: {platforms: {photon: false}}", "platforms: &alias {photon: {enabled: true}}", "platforms: {}\nplatforms: {}"] {
                ...
                do {
                    _ = try await registry.setPlatformEnabled(workspacePath: root.path, profileName: "selected", platform: "photon", enabled: false)
                    XCTFail("Malformed YAML must be rejected.")
                } catch {}
