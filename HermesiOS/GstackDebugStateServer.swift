...
public struct BridgeMutationResult {
    public let ok: Bool
    public let body: JSONDict

    public init(ok: Bool, body: JSONDict = [:]) {
        self.ok = ok
        self.body = body
    }

    public static func success(_ body: JSONDict = [:]) -> BridgeMutationResult {
        BridgeMutationResult(ok: true, body: body)
    }

    public static func failure(_ error: String, _ body: JSONDict = [:]) -> BridgeMutationResult {
        var payload = body
        payload["error"] = error
        return BridgeMutationResult(ok: false, body: payload)
    }
}
...
public final class StateServer {
    ...
    public func register(buildId: String, accessorHash: String, atomicRestore: AtomicRestoreFn? = nil) {
    ...
    private func handleMutation(connection: NWConnection, request: ParsedRequest, op: String) {
        ...
        let result = MutationBridge.dispatch(op: op, payload: payload)
        var body = result.body
        body["op"] = op
        body["ok"] = result.ok
        send(connection: connection, status: result.ok ? 200 : 400, body: body)
    }
...
public enum MutationBridge {
    public static var resolver: (String, JSONDict) -> BridgeMutationResult = { _, _ in .failure("mutation_bridge_not_installed") }
    static func dispatch(op: String, payload: JSONDict) -> BridgeMutationResult { resolver(op, payload) }
}
